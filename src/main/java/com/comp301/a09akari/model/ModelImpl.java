...
public class ModelImpl implements Model {
  ...
  public boolean isLit(int r, int c) {
    if (r >= library.getPuzzle(index).getHeight()
        || c >= library.getPuzzle(index).getWidth()
        || r < 0
        || c < 0) {
      throw new IndexOutOfBoundsException();
    }
    ...
    i = c;
    while (i <= getActivePuzzle().getWidth() - 1
        && getActivePuzzle().getCellType(r, i) == CellType.CORRIDOR) {
      if (isLamp(r, i)) {
        return true;
      }
      i++;
    ...
    j = r;
    while (j <= getActivePuzzle().getHeight() - 1
        && getActivePuzzle().getCellType(j, c) == CellType.CORRIDOR) {
      if (isLamp(j, c)) {
        return true;
      }
      j++;
  ...
  public boolean isLamp(int r, int c) {
    if (r < 0
        || c < 0
        || r >= library.getPuzzle(index).getHeight()
        || c >= library.getPuzzle(index).getWidth()) {
      throw new IndexOutOfBoundsException();
    }
    if (library.getPuzzle(index).getCellType(r, c) != CellType.CORRIDOR) {
